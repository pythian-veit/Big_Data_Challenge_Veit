...
def create_cluster(dataproc, project, cluster_name, zone, init_actions):
...
def submit_pyspark_job(dataproc, project, cluster_name, spark_packages, bucket_name, filename):
...
def main(project_id, zone, cluster_name, init_actions, spark_packages, bucket_name, upload_src, pyspark_file=None):
    dataproc = get_client()
    ...
    try:
        ...
        create_cluster(dataproc, project_id, cluster_name, zone, init_actions)
        wait_for_cluster_creation(dataproc, project_id, cluster_name, zone)
           
        if not upload_src:
            upload_src = 'n'
        
        if upload_src.lower() == 'y':
        ...
        job_id = submit_pyspark_job(
            dataproc, project_id, cluster_name, spark_packages, bucket_name, spark_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--init_actions', help='A list of scripts to be executed during initialization of the cluster.')
    parser.add_argument(
        '--spark_packages', help='Additional packages for Spark')
    parser.add_argument(
        '--gcs_bucket', help='Bucket to upload Pyspark file to')
    parser.add_argument(
        '--upload_src', help='Upload pyspark file to bucket (y) or file is already in bucket (n). Defaults to n')
    parser.add_argument(
    ...
    main(
        ...
        args.cluster_name, args.init_actions, args.spark_packages, args.gcs_bucket, args.upload_src, args.pyspark_file)